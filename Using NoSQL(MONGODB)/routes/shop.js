...
router.route('/add-products').get((req, res) => {
    res.render("pages/add-products");
}).post(async(req, res) => {

    // validation here
    if (!req.body.title || !req.body.description || !req.body.price || !req.body.imageURL) {
...
router.route("/productUpdate/:id").get(async(req, res) => {
...
router.post("/cartDelete/:id", (req, res) => {
    req.user.removeCart(req.params.id).then(result => res.redirect('/cart')).catch(e => console.log(e))
});
